...
-export([get_db_type/0, update/5, update_t/4,
     ...
     add_spool_sql_v2/3,insert_spool_sql_v2/4]).
...
get_password(LServer, LUser) ->
    ejabberd_sql:sql_query(
      LServer,
        [<<"select password from host_users where user_id= '">>,LUser,
            <<"' and host_id = (select id from host_info where host = '">>,LServer,<<"');">>]).
%        [<<"select password from host_users where user_id= '">>,LUser,<<"' and host id = ;">>]).
...
insert_spool_sql_v2(LServer,FUser,LUser, XML) -> 
    ejabberd_sql:sql_query(
        LServer,[<<"insert into spool(from_username,username, xml) values ('">>,FUser,<<"','">>,LUser,<<"','">>,XML,<<"');">>]). 
...
%	end,
    ...
    Result =
        ejabberd_sql:sql_query(LServer,
            [<<"select username ,xml from spool where username = '">>,LUser,<<"' order by seq;">>]),
            
        ejabberd_sql:sql_query(LServer,
            [<<"delete from spool where username='">>,LUser,<<"';">>]),
    Result.
