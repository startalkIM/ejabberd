...
-spec sql_query(sql_query()) -> sql_query_result().
sql_query(Query) -> sql_query(?SERVER_KEY, Query).

-spec sql_query(binary(), sql_query()) -> sql_query_result().
sql_query(Host, Query) ->
    check_error(sql_call(Host, {sql_query, Query}), Query).

sql_query_use_module(Query, Module) -> sql_query_use_module(?SERVER_KEY, Query, Module).

sql_query_use_module(Host, Query,Module) ->
    check_error(sql_call_use_module(Host,{sql_query, Query},Module),Query).
...
sql_transaction(Queries) -> sql_transaction(?SERVER_KEY, Queries).
