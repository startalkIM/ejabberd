...
do_route(From, To, Packet) ->
    ...
    case find_connection(From, To) of
      {atomic, Pid} when is_pid(Pid) ->
          case qtalk_public:check_pid_alive(Pid) of
          true ->    
            send_s2s_packet(From,To,Packet,Pid);
          _ ->
...
update_s2s_mapperd_host(Server) ->
    case catch qtalk_sql:get_s2s_host_info(Server) of
...
send_s2s_packet(From,To,Packet,Pid) ->
    ...
    NewPacket2 =
        case Name of
        <<"message">> ->
            insert_s2s_msg(MyServer,From,To,NewPacket);
        _ ->
            NewPacket
        end,
...
insert_s2s_msg(MyServer,From,To,Packet) ->
    #xmlel{name = Name, attrs = Attrs, children =  Els} = Packet,
    {NewPacket,Now}  =
    case fxml:get_attr_s(<<"msec_times">>, Attrs)  of
        <<"">> ->
                Time = qtalk_public:get_exact_timestamp(),
                {ejabberd_sm:add_msectime_to_packet(Name,Attrs,Els,Time), Time};
         T when is_binary(T) ->
                {Packet,binary_to_integer(T)};
        _ ->
                {Packet,qtalk_public:get_exact_timestamp()}
    end,
    ...
    Mtype when Mtype == <<"normal">>; Mtype == <<"chat">> ; Mtype == <<"consult">> ->
        ...
        true ->
            case fxml:get_tag_attr_s(<<"id">>,fxml:get_subtag(NewPacket,<<"body">>)) of
            <<"">> ->
                    Bid = list_to_binary("ad" ++ binary_to_list(randoms:get_string()) ++ integer_to_list(qtalk_public:get_exact_timestamp())),
                    NewPacket1 = qtalk_public:add_body_id(NewPacket,Bid),
                    ejabberd_sm:insert_chat_msg(MyServer,From#jid.user,To#jid.user,From#jid.lserver,To#jid.lserver, 
                        fxml:element_to_binary(NewPacket1),Mbody,Bid,Now),
                    NewPacket1;
            Msg_id ->
                ejabberd_sm:insert_chat_msg(MyServer,From#jid.user,To#jid.user,From#jid.lserver,To#jid.lserver, 
                        fxml:element_to_binary(NewPacket),Mbody,Msg_id,Now),
                NewPacket
            end;
        _ ->
            NewPacket
