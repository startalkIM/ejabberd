...
init([Opts]) ->
    {ok, {{one_for_one, 10, 1},
                               lists:map(fun({Proc, Option}) ->
                                                 case lists:keyfind(name, 1, Option) of
                                                     false -> {Proc, {kafka_producer, start_link, [[{name, list_to_binary(atom_to_list(Proc))}|Option]]}, transient, 2000, worker, [kafka_producer]};
                                                     _ -> {Proc, {kafka_producer, start_link, [Option]}, transient, 2000, worker, [kafka_producer]}
                                                 end
                                         end, Opts)}}.
