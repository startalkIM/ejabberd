...
sql_call(Host, Msg) ->
    case get(?STATE_KEY) of
      undefined ->
        case mod_pg_odbc:get_random_pid(Host) of
          none -> {error, <<"Unknown Host">>};
         Pid ->
			do_sql_call(Pid, {sql_cmd, Msg, now()})
          end;
      _State -> ?DEBUG("need otp ~n",[]),nested_op(Msg)
    end.

do_sql_call(Pid, Message) ->
	try (?GEN_FSM):sync_send_event(Pid, Message, ?TRANSACTION_TIMEOUT)
	catch
		Type:Error ->
            ?ERROR_MSG("call pg_odbc error for ~p~n", [{Type, Error}]),
            {error, Error}
    end.
...
escape_like($%) -> <<"\\%">>;
escape_like($_) -> <<"\\_">>;
escape_like(C) when is_integer(C), C >= 0, C =< 255 -> escaped(C).
...
connecting(connect, #state{options = Options} = State) ->
      ...
      case ConnectRes of
          {ok, Ref} ->
              ...
              ?INFO_MSG("Connecting to pgsql Server [~p] DB [~p] Pid [~p]  sucess~n",[Server,DB,self()]),
              mod_pg_odbc:add_pid(Host, self()),
              {next_state, session_established,
               State#state{db_ref = Ref,
                           pending_requests = {0, queue:new()}}};
          {error, Reason} ->
              StartInterval = proplists:get_value(start_interval, Options, ?START_INTERVAL),
              ?ERROR_MSG("connection to pgsql Server [~p] DB [~p] failed Reason: ~p~n** ", [Server,DB,Reason]),
...
terminate(Reason, _StateName, #state{options = Options} = State) ->
    ?ERROR_MSG("pg_odbc terminate for ~p, and the options is ~p~n", [Reason, Options]),
    DbType = proplists:get_value(dbtype, Options, ?DEFAULT_DB), 
    Host = proplists:get_value(host, Options, <<"localhost">>), 
    mod_pg_odbc:remove_pid(Host, self()),
    case DbType of
